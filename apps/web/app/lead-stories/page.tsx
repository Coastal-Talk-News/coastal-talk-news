...
interface LeadStoriesPageProps {
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    return buildMetadata({
      ...
      locale,
      origin,
      title: dictionary.leadStories.title,
      description: dictionary.leadStories.description,
      path: '/lead-stories',
    });
  } catch {
    return {};
...
export default async function LeadStoriesPage({
  searchParams,
}: LeadStoriesPageProps) {
  const page = parsePage((await searchParams).page);
  const locale = await getLocale();
  const dictionary = getDictionary(locale);

  const { articles, meta } = await getArticlesByPriority('LEAD_STORY', {
    page,
    ...
    <div className="py-5 sm:py-6">
      ...
      <header className="mb-6">
        ...
        <p className="text-ink-muted mt-2 max-w-2xl leading-relaxed">
          {dictionary.leadStories.description}
        </p>
      </header>
      ...
      {articles.length === 0 ? (
        <EmptyState
          variant="page"
          title={dictionary.leadStories.noStoriesTitle}
          description={dictionary.leadStories.noStoriesDescription}
        />
      ) : (
        <>
          {isFirstPage && lead && (
          ...
          <Pagination
            ...
            href={(target) =>
              target <= 1 ? '/lead-stories' : `/lead-stories?page=${target}`
            }
            locale={locale}
          />
        </>
